...
export function CargoPage({ lockedView }: { lockedView?: 'incoming' | 'outgoing' } = {}) {
  ...
  useEffect(() => {
    ...
    setCargoViewTab(lockedView)
    setTypeFilter(lockedView)
    setCreateCargoType(lockedView === 'incoming' ? 'INCOMING' : 'OUTGOING')
  }, [lockedView])
  ...
  const viewTitle = lockedView === 'incoming'
    ...
    : lockedView === 'outgoing'
      ? 'Kargo Takibi - Giden'
      : 'Kargo Takibi'
  const viewDescription = lockedView === 'incoming'
    ? 'Sadece gelen kargo kayitlarini yonetin ve takip edin.'
    : lockedView === 'outgoing'
      ? 'Sadece giden kargo kayitlarini yonetin ve takip edin.'
      : 'Tum kargo gonderimlerini yonetin ve takip edin.'
  ...
  const filteredCargos = cargos.filter(cargo => {
    ...
    const matchesSearch =
      ...
      String(cargo.sender || '').toLowerCase().includes(lowerSearch) ||
      (cargo.type === 'outgoing' && String(cargo.receiver || '').toLowerCase().includes(lowerSearch)) ||
      String(cargo.cargoCompany || '').toLowerCase().includes(lowerSearch) ||
  ...
  const statsSource = cargos.filter((c) =>
    cargoViewTab === 'incoming' ? c.type === 'incoming' : c.type === 'outgoing'
  )
  const globalIncomingCount = cargos.filter((c) => c.type === 'incoming').length
  const globalOutgoingCount = cargos.filter((c) => c.type === 'outgoing').length

  const stats = {
    total: statsSource.length,
    incoming: globalIncomingCount,
    outgoing: globalOutgoingCount,
    inTransit: statsSource.filter(c => c.status === 'in_transit').length,
    delivered: statsSource.filter(c => c.status === 'delivered').length,
    repair: statsSource.filter(c => c.recordStatus === 'device_repair').length,
    readyToShip: statsSource.filter(c => c.recordStatus === 'ready_to_ship').length,
    onHold: statsSource.filter(c => c.recordStatus === 'on_hold').length,
    closed: statsSource.filter(c => c.recordStatus === 'closed').length,
  }
    ...
    <div className="container mx-auto py-6">
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          {isLockedView ? (
            <Button onClick={() => openCreateCargoDialog(lockedView === 'incoming' ? 'INCOMING' : 'OUTGOING')}>
              <Plus className="mr-2 h-4 w-4" />
              {lockedView === 'incoming' ? 'Yeni Gelen Kargo Girisi' : 'Yeni Giden Kargo Girisi'}
            </Button>
          ) : (
            <>
